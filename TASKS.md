...
- [ ] `npm init`, установка зависимостей (см. [STACK.md](STACK.md)), TS-конфиг, ESLint/Prettier.
- [ ] `.env.example` (MongoDB/Redis поднимаются локально, **без Docker**).
- [ ] `src/config/` — загрузка и валидация env (падать при старте, если нет секрета).
- [ ] `src/app.ts` (Express, middleware) и `src/server.ts` (connect + listen).
- [ ] `GET /health`.
...
- [ ] `Merchant`: `_id`, `feePercent` (basis points / Decimal).
- [ ] `Invoice`: `amount`, `currency`, `merchantId`, `fee`, `amountToReceive`,
      `status` (`pending|paid|failed`), `settledAt?`, таймстемпы. Суммы — целые минорные единицы.
- [ ] `WebhookEvent`: `nonce` (**unique index**), `invoiceId`, `status`, `receivedAt`.
...
- [ ] `computeFee(amount, feePercent)` и `computeAmountToReceive(amount, fee)` в минорных единицах.
- [ ] Явное правило округления (зафиксировать в README).
...
- [ ] DTO-валидация (`amount` > 0, `currency`, существующий `merchantId`).
- [ ] Расчёт `fee`/`amountToReceive`, сохранение со статусом `pending`.
- [ ] Ответ: `invoiceId`, `amount`, `fee`, `amountToReceive`, `status`.
...
- [ ] Возврат документа; 404 если не найден; 400 на невалидный id.
...
- [ ] Middleware, сохраняющий `req.rawBody` (через `express.json({ verify })`),
      применять к `/webhook`.
...
- [ ] `verifySignature(rawBody, secret, headerSig)` — `HMAC-SHA256`, `timingSafeEqual`.
- [ ] Middleware: нет/битая подпись → `401`, без побочных эффектов.
...
- [ ] Проверка `X-Timestamp` в пределах окна `WEBHOOK_TIMESTAMP_TOLERANCE_SEC` → иначе `401`.
- [ ] `X-Nonce`: Redis `SET nonce 1 NX EX <TTL>`; если уже есть → повтор.
...
- [ ] DTO `{ invoiceId, status }` (`paid|failed`).
- [ ] Redis distributed lock по `invoiceId` (сериализация конкурентных вебхуков).
- [ ] Атомарный переход: `findOneAndUpdate({_id, status:'pending'}, {status, settledAt})` —
      зачисление выполняется только если апдейт реально сменил статус.
- [ ] Запись `WebhookEvent`; повтор по уже обработанному nonce/событию → noop.
- [ ] При `paid` — побочный эффект «зачисление» **в виде заглушки** (поле `settledAt`/лог,
...
- [ ] Глобальный error-handler, единый формат `{ error: { code, message } }`.
- [ ] Структурное логирование (pino) запросов и ошибок.
- [ ] Корректные коды: 400/401/404/409/422/500.
...
- [ ] Подпись (валид/невалид).  [ ] Идемпотентность вебхука.  [ ] Расчёт комиссии.
- [ ] Конкурентные дубли (Promise.all нескольких одинаковых вебхуков).
- [ ] Инфраструктура тестов: `mongodb-memory-server`, тестовый/мок Redis, `supertest`.
...
- [ ] Инструкция запуска (`npm i`, локальные Mongo/Redis, `npm run dev`, `npm test`) — **без Docker**.
- [ ] Описание переменных окружения и пример запроса с подписью (curl).
- [ ] **Принятые допущения** и **что не доделано + как доделать**.
- [ ] Залить в GitHub/GitLab, прислать ссылку.
...
- [ ] Денежные расчёты точны (без float).
- [ ] HMAC-SHA256 от тела + timingSafeEqual.
- [ ] Защита от replay: timestamp + уникальный nonce.
- [ ] `paid` зачисляет ровно один раз при любых дублях/гонках.
- [ ] MongoDB (Mongoose) + Redis задействованы по назначению.
- [ ] Тесты: подпись, идемпотентность, комиссия (+ конкурентность).
- [ ] README с запуском, тестами, допущениями.
